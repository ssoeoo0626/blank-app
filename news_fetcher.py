...
def build_search_query(company, keyword, domain):
        ...
        if not candidate_df.empty:
            ...
            for _, s in candidate_df.iterrows():
                ...
                rows.append({
                    ...
                    "도메인": domain,
                    "우선순위": priority,
                    "검색쿼리": build_search_query(company, keyword, domain),
                })
...
def fetch_google_news_rss(query, timeout=20):
...
def fetch_news_from_query_table(query_df, days_back=30):
